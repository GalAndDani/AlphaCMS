$(function() {
    ...
    Array.prototype.slice.call(document.querySelectorAll(".switch")).forEach(function(t) { new Switchery(t, { color: "#4CAF50" }) }), $(".daterange-ranges").daterangepicker({ startDate: moment().subtract(29, "days"), endDate: moment(), minDate: "01/01/2012", maxDate: "12/31/2016", dateLimit: { days: 60 }, ranges: { Today: [moment(), moment()], Yesterday: [moment().subtract(1, "days"), moment().subtract(1, "days")], "Last 7 Days": [moment().subtract(6, "days"), moment()], "Last 30 Days": [moment().subtract(29, "days"), moment()], "This Month": [moment().startOf("month"), moment().endOf("month")], "Last Month": [moment().subtract(1, "month").startOf("month"), moment().subtract(1, "month").endOf("month")] }, opens: "left", applyClass: "btn-small bg-slate-600 btn-block", cancelClass: "btn-small btn-default btn-block", format: "MM/DD/YYYY", locale: { direction: "rtl" } }, function(t, e) { $(".daterange-ranges span").html(t.format("MMMM D") + " - " + e.format("MMMM D")) }), $(".daterange-ranges span").html(moment().subtract(29, "days").format("MMMM D") + " - " + moment().format("MMMM D")),
        function(t, e) {
        ...
        function(t, e) {
            ...
            function n() {
                ...
                e = e - i.top - i.bottom,
                l = d3.tip().attr("class", "d3-tip").html(function(t) { return "<ul class='list-unstyled mb-5'><li><div class='text-size-base mt-5 mb-5'><i class='icon-circle-right2 position-left'></i>" + t.name + " app</div></li><li>Sales: &nbsp;<span class='text-semibold pull-right'>" + t.value + "</span></li><li>Revenue: &nbsp; <span class='text-semibold pull-right'>$" + (25 * t.value).toFixed(2) + "</span></li></ul>" }),
        ...
        function() { d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function(t, e) {
                function a() { width = l.node().getBoundingClientRect().width - margin.left - margin.right, gridSize = width / new Date(e[e.length - 1].date).getHours(), height = (rowGap + gridSize) * d3.max(r, function(t, e) { return e + 1 }) - margin.top, u.attr("width", width + margin.left + margin.right).attr("height", height + margin.bottom), p.attr("width", width + margin.left + margin.right).attr("height", height + margin.bottom), o.range([0, width]), p.selectAll(".hour-group").attr("transform", function(t, e) { return "translate(0, " + (gridSize + rowGap) * e + ")" }), p.selectAll(".heatmap-hour").attr("width", gridSize).attr("height", gridSize).attr("x", function(t, e) { return o(t.date) }), p.selectAll(".legend-group").attr("transform", "translate(" + (width / 2 - buckets * gridSize / 2) + "," + (height + margin.bottom - margin.top) + ")"), p.selectAll(".app-label").attr("x", width), p.selectAll(".heatmap-legend-item").attr("width", gridSize).attr("x", function(t, e) { return gridSize * e }), p.selectAll(".min-legend-value").attr("x", buckets * gridSize + 10) } var n = d3.nest().key(function(t) { return t.app }),
                    r = n.entries(e),
                ...
                f.append("text").attr("class", "app-label").attr("x", width).attr("y", -(margin.top / 2)).style("text-anchor", "end").text(function(t, e) { return t.key }), f.append("text").attr("class", "sales-count").attr("x", 0).attr("y", -(margin.top / 2)).style("text-anchor", "start").text(function(t, e) { return d3.sum(t.values, function(t) { return t.value }) + " sales today" }); var h = f.selectAll(".heatmap-hour").data(function(t) { return t.values }).enter().append("rect").attr("x", function(t, e) { return o(t.date) }).attr("y", 0).attr("class", "heatmap-hour").attr("width", gridSize).attr("height", gridSize).style("fill", "#fff").style("stroke", "#fff").style("cursor", "pointer").style("shape-rendering", "crispEdges");
                ...
                y.selectAll(".heatmap-legend").data([0].concat(c.range(colors.reverse()).quantiles()), function(t) { return t }).enter().append("g").attr("class", "heatmap-legend").append("rect").attr("class", "heatmap-legend-item").attr("x", function(t, e) { return gridSize * e }).attr("y", -8).attr("width", gridSize).attr("height", 5).style("stroke", "#fff").style("shape-rendering", "crispEdges").style("fill", function(t, e) { return colors[e] }), y.append("text").attr("class", "min-legend-value").attr("x", buckets * gridSize + 10).attr("y", -2).style("text-anchor", "start").style("font-size", 11).style("fill", "#999").text(m), y.append("text").attr("class", "max-legend-value").attr("x", -10).attr("y", -2).style("text-anchor", "end").style("font-size", 11).style("fill", "#999").text(g), $(window).on("resize", a), $(".sidebar-control").on("click", a) }) }(),
        ...
        function(t, e, a) { var n = d3.select(t),
            ...
            d3.json("assets/demo_data/dashboard/monthly_sales.json", function(t, e) {
                ...
                c.domain(d3.extent(e, function(t, e) { return t.date })), u.domain([0, d3.max(e, function(t) { return t.value })]), o.append("path").datum(e).attr("class", "d3-area").style("fill", a).attr("d", d).transition().duration(1e3).attrTween("d", function() { var t = d3.interpolateArray(f, e); return function(e) { return d(t(e)) } }), $(window).on("resize", p), $(".sidebar-control").on("click", p) }) }("#messages-stats", 40, "#5C6BC0"), t("#new-visitors", "line", 30, 35, "basis", 750, 2e3, "#26A69A"), t("#new-sessions", "line", 30, 35, "basis", 750, 2e3, "#FF7043"), t("#total-online", "line", 30, 35, "basis", 750, 2e3, "#5C6BC0"), t("#server-load", "area", 30, 50, "basis", 750, 2e3, "rgba(255,255,255,0.5)"),
        function(t, e) {
        ...
        function(t, e) {
            var a = [{ browser: "Google Adwords", icon: "<i class='icon-google position-left'></i>", value: 1047, color: "#66BB6A" }, { browser: "Social media", icon: "<i class='icon-share4 position-left'></i>", value: 2948, color: "#9575CD" }, { browser: "Youtube video", icon: "<i class='icon-youtube position-left'></i>", value: 3909, color: "#FF7043" }],
                n = d3.select(t),
                r = e / 2 - 2,
        ...
        function(t, e) {
            ...
            function n(t) { var e = .2 * d3.max(t.ranges); return function(t) { return Math.max(0, t + e * (Math.random() - .5)) } }! function() {
                ...
                d3.bullet = function() {
                    function l(t) { t.each(function(t, e) {
                            ...
                            F.select("line").attr("y1", h + 3).attr("y2", 7 * h / 6 + 3), F.select("text").attr("y", 7 * h / 6 + 4), M.exit().transition().duration(c).attr("transform", n(v)).style("opacity", 1e-6).remove(), $(window).on("resize", a), $(".sidebar-control").on("click", a) }), d3.timer.flush() } var o = "left",
                        d = !0,
                        c = 750,
                        u = t,
                        p = e,
            ...
            d3.json("assets/demo_data/dashboard/bullets.json", function(t, n) {
                function o() { s = r.node().getBoundingClientRect().width - i.left - i.right, d.attr("width", s + i.left + i.right), c.attr("width", s + i.left + i.right), c.selectAll(".bullet-title").attr("x", s) } if (t) return console.error(t); var d = r.selectAll("svg").data(n).enter().append("svg"),
                    ...
                    u = c.append("g").style("text-anchor", "end");
                u.append("text").attr("class", "bullet-title").attr("x", s).attr("y", -10).text(function(t) { return t.title }), u.append("text").attr("class", "bullet-subtitle").attr("y", -10).style("text-anchor", "start").text(function(t) { return t.subtitle }).style("opacity", 0).transition().duration(500).delay(500).style("opacity", 1); var p = function() { c.datum(a).call(l.duration(750)) },
                    f = [];
                ...
                new Switchery(h), h.onchange = function() { if (h.checked) f.push(setInterval(function() { p() }, 5e3));
                        ...
                        for (var t = 0; t < f.length; t++) clearInterval(f[t]) }, $(window).on("resize", o), $(".sidebar-control").on("click", o) }) }("#bullets", 80), $(".table tr").each(function(t) { var e = $(this).find(".letter-icon-title"),
